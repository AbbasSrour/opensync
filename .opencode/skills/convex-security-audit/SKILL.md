...
description: Use when deep security review patterns for authorization logic, data access boundaries, action isolation, rate limiting, and protecting sensitive operations
---

# Convex Security Audit

> Migrated from `.claude/skills/convex-security-audit.md`.
...
export const getSharedDocument = query({
  ...
  handler: async (ctx, args) => {
    ...
    const access = await ctx.db
      ...
      .withIndex("by_doc_and_user", (q) => q.eq("documentId", args.docId).eq("userId", user._id))
      .unique();
...
export const getAuditLogs = query({
  ...
  handler: async (ctx, args) => {
    ...
    if (args.resourceType) {
      query = query.withIndex("by_resource_type", (q) => q.eq("resourceType", args.resourceType));
    }

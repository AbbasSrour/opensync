---
name: opensync-plugin-development
description: Use when working with opensync plugin development.
---

# Opensync Plugin Development

> Migrated from `.claude/skills/opensync-plugin-development.md`.
...
export const syncSession = (data: SyncSessionData) => request("/sync/session", "POST", data);
```
...
async function status() {
  ...
  const result = await testConnection();
  console.log(result.success ? "Connected to OpenSync" : `Error: ${result.error}`);
}

...
export const semanticSearch = action({
  ...
  handler: async (ctx, { query: searchQuery, limit = 10 }): Promise<SessionSearchResult[]> => {
    ...
    // Vector search
    const results: { _id: Id<"sessionEmbeddings">; _score: number }[] = await ctx.vectorSearch(
      "sessionEmbeddings",
      "by_embedding",
      {
    ...
    // Load sessions
    const sessions: SessionSearchResult[] = await ctx.runQuery(
      internal.search.loadSessionsFromEmbeddings,
      {
...
export const semanticSearchMessages = action({
  ...
  handler: async (
    ctx,
    { query: searchQuery, sessionId, limit = 20 },

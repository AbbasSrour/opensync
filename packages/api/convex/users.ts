...
export const updateEnabledAgents = mutation({
  ...
  handler: async (ctx, { enabledAgents }) => {
    ...
    const normalizedAgents = [
      ...new Set(enabledAgents.map((a) => (a === "cursor" ? "cursor-sync" : a))),
    ];
...
export const deleteAllData = mutation({
  ...
  handler: async (ctx) => {
    ...
    if (user.deletionStatus === "pending" || user.deletionStatus === "in_progress") {
      return {
        started: false,
...
export const orchestrateBatchDeletion = internalAction({
  ...
  handler: async (ctx, { userId, deleteUser }) => {
    ...
    try {
      ...
      while (hasMoreParts) {
        const result = await ctx.runMutation(internal.users.deletePartsBatch, { userId });
      ...
      while (hasMoreMessages) {
        const result = await ctx.runMutation(internal.users.deleteMessagesBatch, { userId });
      ...
      while (hasMoreSessions) {
        const result = await ctx.runMutation(internal.users.deleteSessionsBatch, { userId });
      ...
      while (hasMoreSessionEmbeddings) {
        const result = await ctx.runMutation(internal.users.deleteSessionEmbeddingsBatch, {
          userId,
        });
      ...
      while (hasMoreMessageEmbeddings) {
        const result = await ctx.runMutation(internal.users.deleteMessageEmbeddingsBatch, {
          userId,
        });
      ...
      while (hasMoreDailyWrapped) {
        const result = await ctx.runMutation(internal.users.deleteDailyWrappedBatch, { userId });
      ...
      while (hasMoreApiLogs) {
        const result = await ctx.runMutation(internal.users.deleteApiLogsBatch, { userId });
  ...
  handler: async (ctx, { userId }) => {
    ...
    if (user?.deletionProgress) {
      await ctx.db.patch(userId, {
        deletionProgress: {
          ...user.deletionProgress,
          sessionEmbeddings: user.deletionProgress.sessionEmbeddings + embeddings.length,
        },
      });
    }
  ...
  handler: async (ctx, { userId }) => {
    ...
    if (user?.deletionProgress) {
      await ctx.db.patch(userId, {
        deletionProgress: {
          ...user.deletionProgress,
          messageEmbeddings: user.deletionProgress.messageEmbeddings + embeddings.length,
        },
      });
    }
...
export const deleteAccount = action({
  ...
  handler: async (ctx): Promise<{ started: boolean; message: string; error?: string }> => {
    const identity = await ctx.auth.getUserIdentity();
    if (!identity) {
      return {
        started: false,
    ...
    // Get the user record
    const user: { _id: Id<"users">; workosId: string } | null = await ctx.runMutation(

  ...
  handler: async (ctx, args) => {
    ...
    if (args.tags && args.tags.length > 0) {
      sessions = sessions.filter(
        (s) => s.evalTags && args.tags!.some((tag) => s.evalTags!.includes(tag)),
      );
    }
    if (args.model) {
    ...
    const bySource = {
      opencode: sessions.filter((s) => s.source === "opencode" || !s.source).length,
      ...
      cursor: sessions.filter((s) => s.source === "cursor-sync" || s.source === "cursor").length,
    };
    ...
    const byStatus = {
      ...
      needsReview: sessions.filter((s) => s.evalStatus === "needs_review").length,
...
export const previewExport = query({
  ...
  handler: async (ctx, args) => {
    ...
    if (args.sessionIds === "all") {
      ...
      const fetched = await Promise.all(args.sessionIds.map((id) => ctx.db.get(id)));
      sessions = fetched.filter((s) => s && s.userId === user._id && s.evalReady) as NonNullable<
    ...
    if (noModelSessions.length > 0) {
      warnings.push(`${noModelSessions.length} session(s) have no model specified`);
    }
    if (sessions.length > 1000) {
      warnings.push("Large export (1000+ sessions) may take longer to generate");
    }
  ...
  handler: async (ctx, args) => {
    ...
    if (args.format === "deepeval") {
      ...
      for (const session of sessions) {
        ...
        if (messages.length === 0) {
          skippedCount++;
          warnings.push(`Session ${session.externalId.slice(0, 8)}: No messages after filtering`);
          continue;
        }
        ...
        if (turnMode === "full") {
          ...
          if (userMessages.length > 0 && assistantMessages.length > 0) {
            testCases.push({
              input: anonymize(userMessages.map((m) => m.textContent || "").join("\n\n")),
              actual_output: anonymize(
                assistantMessages.map((m) => m.textContent || "").join("\n\n"),
              ),
              expected_output: session.expectedOutput ? anonymize(session.expectedOutput) : null,
              context: [],
          ...
          if (lastUserIdx !== undefined && lastUserIdx + 1 < messages.length) {
            ...
            if (assistantMsg.role === "assistant") {
              testCases.push({
                ...
                expected_output: session.expectedOutput ? anonymize(session.expectedOutput) : null,
                context: messages.slice(0, lastUserIdx).map((m) => anonymize(m.textContent || "")),
                retrieval_context: null,
                metadata: {
                  session_id: session.externalId,
                  model: session.model || "unknown",
              ...
              if (nextAssistantIdx !== -1) {
                ...
                testCases.push({
                  ...
                  context: args.options.includeToolCalls
                    ? messages.slice(0, i).map((m) => anonymize(m.textContent || ""))
                    : [],
                  retrieval_context: null,
                  metadata: {
                    session_id: session.externalId,
          ...
          if (msg.role === "user") {
            ...
            if (nextAssistantIdx !== -1) {
              ...
              const evalCase: OpenAIEvalCase = {
                input: [...context],
                ideal: anonymize(session.expectedOutput || response.textContent || ""),
                metadata: {
                  session_id: session.externalId,
                  model: session.model || "unknown",
        ...
        for (let i = 0; i < messages.length; i++) {
          ...
          if (msg.role === "user") {
            ...
            if (nextAssistantIdx !== -1) {
              ...
              } else if (content.includes("explain") || content.includes("what")) {
              ...
              } else if (session.evalStatus === "golden" || session.evalStatus === "correct") {
                testCase.assert = [
                  {
                    type: "llm-rubric",
    ...
    for (const session of sessions) {
      ...
      if (session.projectName) {
        lines.push(`PROJECT: ${args.options.anonymizePaths ? "project" : session.projectName}`);
      }
      lines.push("=".repeat(80));
      lines.push("");
    ...
    const manifest = {
      ...
      sources: {
        ...
        "factory-droid": sessions.filter((s) => s.source === "factory-droid").length,
        "codex-cli": sessions.filter((s) => s.source === "codex-cli").length,
        "cursor-sync": sessions.filter((s) => s.source === "cursor-sync" || s.source === "cursor")
          .length,
      },

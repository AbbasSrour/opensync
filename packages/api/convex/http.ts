  ...
  handler: httpAction(async (ctx, request) => {
    ...
    try {
      ...
      const minTokens = url.searchParams.get("min_tokens")
        ? parseInt(url.searchParams.get("min_tokens")!)
        : undefined;
      ...
      if (tags && tags.length > 0) {
        filteredSessions = filteredSessions.filter(
          (s: any) => s.evalTags && tags.some((tag: string) => s.evalTags.includes(tag)),
        );
      }
      ...
      const contentType =
        format === "openai" || format === "promptfoo" ? "application/x-ndjson" : "application/json";

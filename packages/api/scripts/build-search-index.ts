/**
 * SCAFFOLDING (orphaned): generates a search-index.json from docs MDX, but nothing
 * imports the output. Carried over from the original repo unchanged.
 *
 * Build-time script to generate search index from MDX files
 * Run: tsc && vp build:search-index
 */
...
async function buildSearchIndex(): Promise<void> {
  ...
  const outputPath = path.resolve(
    import.meta.dirname,
    "../../../apps/website/src/search-index.json",
  );

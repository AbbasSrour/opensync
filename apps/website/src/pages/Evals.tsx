...
const EVAL_STATUS_CONFIG: Record<EvalStatus, { label: string; icon: typeof Star; color: string }> =
  {
...
function StatusBadge({ status }: { status?: EvalStatus }) {
  ...
  return (
    <span className={cn("inline-flex items-center gap-0.5 text-[9px] font-medium", config.color)}>
    ...
    <div className="relative">
      ...
      >
        {value ? (
          ...
          <span className={isDark ? "text-zinc-500" : "text-[#999]"}>Status</span>
        )}
        <ChevronDown className={cn("h-2.5 w-2.5", isDark ? "text-zinc-500" : "text-[#999]")} />
      </button>
        ...
        <>
          ...
          <div
            className={cn(
              ...
              isDark ? "bg-zinc-800 border-zinc-700" : "bg-white border-[#e6e4e1]",
            )}
          >
            {(
...
export function EvalsPage() {
  ...
  const dateFromTs = dateFromFilter ? new Date(dateFromFilter).getTime() : undefined;
  const dateToTs = dateToFilter ? new Date(dateToFilter + "T23:59:59").getTime() : undefined;
  ...
  const previewData = useQuery(
    ...
    showExportModal
      ? {
          sessionIds: selectedSessions.size > 0 ? Array.from(selectedSessions) : "all",
          source: sourceFilter,
          model: modelFilter,
          dateFrom: dateFromTs,
          dateTo: dateToTs,
  ...
  const hasActiveFilters =
    sourceFilter || tagFilter || modelFilter || statusFilter || dateFromFilter || dateToFilter;
  ...
  const handleStatusChange = async (sessionId: Id<"sessions">, status: EvalStatus | undefined) => {
  ...
  return (
    <div className={cn("h-screen flex flex-col overflow-hidden", t.bgPrimary)}>
      {/* Header */}
      <header
        className={cn("h-11 border-b flex items-center px-3 gap-3 shrink-0", t.border, t.bgPrimary)}
      >
        ...
        <div className={cn("flex items-center gap-1.5 text-xs font-medium", t.textPrimary)}>
        ...
        <button
          onClick={toggleTheme}
          className={cn("p-1.5 rounded-md transition-colors", t.bgHover, t.textSubtle)}
        ...
        <div className="relative group">
          <button className={cn("flex items-center gap-1.5 px-1.5 py-0.5 rounded-md", t.bgHover)}>
            <User className={cn("h-3.5 w-3.5", t.textSubtle)} />
            <span className={cn("text-[10px]", t.textSubtle)}>{user?.firstName || "User"}</span>
          </button>
          <div
            className={cn(
      ...
      >
        ...
        <div className="max-w-5xl mx-auto space-y-3">
          ...
          <div className="grid grid-cols-5 gap-2">
            ...
            <StatCard label="Golden" value={stats.byStatus.golden} theme={theme} />
            <StatCard label="Correct" value={stats.byStatus.correct} theme={theme} />
            <StatCard
              label="Review"
          ...
          <div className={cn("p-2 rounded-lg border", t.bgCard, t.border)}>
            <div className="flex flex-wrap items-center gap-1.5">
              ...
              <select
                value={statusFilter || ""}
                onChange={(e) =>
                  setStatusFilter((e.target.value || undefined) as EvalStatus | undefined)
                }
                className={cn(
                  "text-[10px] px-1.5 py-1 rounded border",
                  t.bgInput,
              ...
              {selectedSessions.size > 0 && (
                <div className="flex items-center gap-1.5">
                  <span className={cn("text-[10px]", t.textSubtle)}>{selectedSessions.size}</span>
                  <StatusDropdown
                    value={undefined}
            ...
            {showAdvancedFilters && (
              <div className={cn("flex items-center gap-2 pt-2 mt-2 border-t", t.border)}>
          ...
          {sessions.length === 0 ? (
            <div className={cn("text-center py-12 rounded-lg border", t.bgCard, t.border)}>
              <CheckCircle2 className={cn("h-10 w-10 mx-auto mb-3", t.iconMuted)} />
              <p className={cn("text-xs font-medium mb-1", t.textPrimary)}>No eval sessions yet</p>
              ...
              {sessions.map((session) => (
                <div
                  ...
                  className={cn(
                    ...
                    t.border,
                    selectedSessions.has(session._id) ? t.bgActive : "hover:bg-opacity-50",
                  )}
                >
                  <div className="w-5 shrink-0">
                    <input
                  ...
                  <div
                    className={cn("w-20 shrink-0 truncate hidden sm:block", t.textMuted)}
                  ...
                  <div className="w-16 shrink-0">
                    <StatusDropdown
                      ...
                      onChange={(status) => handleStatusChange(session._id, status)}
                      theme={theme}
                    />
                  </div>
          ...
          >
            ...
            <div
              className={cn(
                ...
                isDark ? "border-zinc-800 bg-zinc-900" : "border-[#e6e4e1] bg-[#faf8f5]",
              )}
            >
              <h3
                className={cn("text-xs font-medium", isDark ? "text-zinc-200" : "text-[#1a1a1a]")}
              >
                Export for Evals
              </h3>
              <button
            ...
            <div className="px-3 py-3 space-y-3">
              ...
              {previewData && (
                <div
                  className={cn(
                    ...
                    isDark ? "border-zinc-800 bg-zinc-800/50" : "border-[#e6e4e1] bg-[#f5f3f0]",
                  )}
                >
                  <div className="grid grid-cols-3 gap-3 text-center">
                    <div>
                      ...
                      <div
                        className={cn("text-[9px]", isDark ? "text-zinc-500" : "text-[#6b6b6b]")}
                      >
                        Sessions
                      </div>
                    </div>
              ...
              {/* Turn mode */}
              {(exportFormat === "deepeval" || exportFormat === "promptfoo") && (
                <div className="space-y-1.5">
                  <label
                    className={cn(
                      "text-[10px] font-medium",
                ...
                <div className="space-y-1">
                  {[
                    ...
                    <label key={opt.key} className="flex items-center gap-1.5">
                      ...
                      />
                      <span
                        className={cn("text-[10px]", isDark ? "text-zinc-400" : "text-[#6b6b6b]")}
                      >
                        {opt.label}
                      </span>
                    </label>
            ...
            >
              ...
              <button
                ...
                disabled={isExporting || !previewData || previewData.sessionCount === 0}
                className={cn(

    ...
    return (
      <div className={cn("min-h-screen flex items-center justify-center", t.bgPrimary)}>
        <div className="text-center">
          <h1 className={cn("text-2xl font-bold mb-2", t.textPrimary)}>Session Not Found</h1>
          <p className={cn("mb-4", t.textMuted)}>This session may be private or deleted.</p>
          <Link
            to="/"
    ...
    <div className={cn("flex gap-3", isUser && "flex-row-reverse")}>
      ...
      <div className={cn("flex-1 max-w-3xl", isUser && "flex flex-col items-end")}>
        <div
          className={cn(
            "rounded-lg p-4",
            isUser ? cn(t.bgUserBubble, t.textPrimary) : cn(t.bgCard, "border", t.border),
          )}
        >
          {showFallback ? (
                ...
                components={{
                  code({ node, inline, className, children, ...props }: any) {
                    ...
                    return !inline && match ? (
                      ...
                      <code
                        className={cn(className, theme === "tan" && "bg-[#e6e4e1] text-[#1a1a1a]")}
                        {...props}
                      >
                        {children}
                      </code>

...
export function WrappedView() {
  ...
  const date =
    todayWrapped?.date || countdownInfo?.currentDate || new Date().toISOString().split("T")[0];

  // Countdown to next generation
  const nextGenTime =
    todayWrapped?.nextGenerationAt ||
    countdownInfo?.nextGenerationAt ||
  ...
  if (!stats || (stats.totalTokens === 0 && stats.totalMessages === 0)) {
    return (
      <div className="h-full flex flex-col items-center justify-center text-center px-4">
        ...
        <h3 className={cn("text-lg font-semibold mb-2", t.textPrimary)}>No Activity Yet</h3>
        <p className={cn("max-w-md", t.textMuted)}>
          Start syncing your coding sessions to see your Daily Wrapped. Activity from the last 24
          hours will appear here.
        </p>
        <div className={cn("mt-6 text-sm", t.textSubtle)}>Next generation at 9:30 AM PT</div>
      </div>
    );
  }
  ...
  return (
    <div className="h-full overflow-y-auto flex flex-col lg:flex-row gap-8 p-4 lg:p-8">
      ...
      >
        <WrappedTemplate designIndex={designIndex} stats={stats} date={date} />
      </div>
      ...
      <div className="lg:w-80 space-y-6">
        ...
        <div className={cn("rounded-xl p-6 shadow-sm border", t.bgCard, t.border)}>
          <h3 className={cn("text-sm font-medium mb-4", t.textMuted)}>Past 24 Hours</h3>
          <div className="space-y-3">
            ...
            <div className="flex justify-between">
              ...
              <span className={cn("font-semibold", t.textPrimary)}>{stats.totalMessages}</span>
            </div>
            <div className="flex justify-between">
          ...
          {stats.topModels.length > 0 && (
            <div className={cn("mt-4 pt-4 border-t", t.border)}>
              <h4 className={cn("text-xs font-medium mb-2", t.textMuted)}>Top Models</h4>
              <div className="space-y-1">
                {stats.topModels.slice(0, 3).map((m: { model: string; tokens: number }) => (
                  <div key={m.model} className="flex justify-between text-sm">
                    <span className={cn("truncate max-w-[60%]", t.textMuted)}>{m.model}</span>
          ...
          {stats.topProviders.length > 0 && (
            <div className={cn("mt-4 pt-4 border-t", t.border)}>
              <h4 className={cn("text-xs font-medium mb-2", t.textMuted)}>Providers</h4>

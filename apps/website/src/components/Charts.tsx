...
export function BarChart({
  ...
  if (data.length === 0) {
    return (
      <div
        className={cn("flex items-center justify-center text-zinc-600 text-sm", className)}
        style={{ height }}
      >
        No data
      </div>
...
export function DonutChart({ data, size = 100, thickness = 12, className }: DonutChartProps) {
    ...
    >
      <div className="flex items-start justify-between">
        <div className="space-y-1">
          <p className={cn("text-xs font-normal", isDark ? "text-zinc-500" : "text-[#6b6b6b]")}>
            {label}
          </p>
          <p className={cn("text-xl font-light", isDark ? "text-zinc-100" : "text-[#1a1a1a]")}>
            {value}
          </p>
          {subValue && (
            <p className={cn("text-xs", isDark ? "text-zinc-600" : "text-[#8b7355]")}>{subValue}</p>
          )}
        </div>
...
export function DataTable<T>({
  ...
  if (data.length === 0) {
    return (
      <div
        className={cn("flex items-center justify-center py-12 text-zinc-600 text-sm", className)}
      >
        {emptyMessage}
      </div>
    );
...
export function StackedBarChart({
  ...
  const maxValue = useMemo(() => {
    return Math.max(...data.map((d) => d.segments.reduce((sum, s) => sum + s.value, 0)), 1);
  }, [data]);
        ...
        {data.map((item, i) => {
          ...
          return (
            ...
            >
              ...
              >
                <div
                  className={cn("font-medium mb-1", isDark ? "text-zinc-100" : "text-[#1a1a1a]")}
                >
                  {item.label}
                </div>
                {item.segments.map((seg, j) => (
                  <div key={j} className="flex items-center gap-2 py-0.5">
                    ...
                    <span className={isDark ? "text-zinc-400" : "text-[#6b6b6b]"}>
                      {seg.label}:
                    </span>
    ...
    <div className={cn("space-y-2", className)}>
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          >
            <svg
              className={cn("w-4 h-4", isDark ? "text-blue-400" : "text-[#EB5601]")}
              viewBox="0 0 24 24"
              fill="none"
              stroke="currentColor"
              strokeWidth="2"
          ...
          <div>
            <p className={cn("text-xs", isDark ? "text-zinc-500" : "text-[#6b6b6b]")}>
              Included Credit
            </p>
...
export function ConsumptionBreakdown({
  ...
  const formatPeriodLabel = (period: string) => {
    if (viewMode === "monthly") {
      ...
      return new Date(parseInt(year), parseInt(month) - 1).toLocaleDateString("en", {
        month: "short",
      });
    }
    ...
    >
      ...
      <div className="px-4 py-4">
        ...
        <StackedBarChart
          ...
          height={180}
          formatValue={
            chartType === "tokens" ? (v) => `${(v / 1000).toFixed(1)}K` : (v) => `$${v.toFixed(2)}`
          }
          theme={theme}
        />
        ...
        <div className="flex flex-wrap gap-3 mt-4">
          {chartType === "tokens" ? (
            <>
              <div className="flex items-center gap-1.5 text-xs">
                <span
                  ...
                  style={{ backgroundColor: isDark ? "#3b82f6" : "#EB5601" }}
                />
              ...
              <div className="flex items-center gap-1.5 text-xs">
                <span
                  className="w-2.5 h-2.5 rounded-sm"
                  style={{ backgroundColor: isDark ? "#22c55e" : "#8b7355" }}
                />
                <span className={isDark ? "text-zinc-400" : "text-[#6b6b6b]"}>
                  Completion Tokens
                </span>
            ...
            tableStats.slice(0, 6).map((s, i) => (
              <div
                key={"model" in s ? s.model : s.project}
          ...
          <thead>
            <tr
              className={cn(
                ...
                isDark ? "border-zinc-800/30 text-zinc-600" : "border-[#e6e4e1] text-[#8b7355]",
              )}
            >
            ...
            {tableStats.slice(0, 5).map((s, i) => {
              ...
              return (
                ...
                >
                  ...
                  <td
                    className={cn(
                      "px-4 py-2.5 text-sm text-right font-medium",

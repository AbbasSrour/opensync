...
<Steps>
  <Step title="Fork on GitHub">
    Visit [github.com/waynesutton/opensync](https://github.com/waynesutton/opensync) and click
    **Fork**.
  </Step>
  ...
  <Step title="Start Convex">
    ```bash vp run @opensync/convex#convex ``` This creates a new Convex project linked to your
    account.
  </Step>
  <Step title="Start frontend">```bash cd apps/website && vp dev ```</Step>
</Steps>
...
```
opensync/
├── packages/convex/src/ # Convex backend
│   ├── schema.ts     # Database schema
│   ├── http.ts       # HTTP endpoints
│   ├── sessions.ts   # Session functions
│   └── ...
├── apps/website/src/ # React frontend

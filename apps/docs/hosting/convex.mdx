...
`vp run @opensync/convex#convex` runs in watch mode. It pushes changes to your development deployment every time you save a file in the `packages/convex/src/` directory.

### 3. Set environment variables

```bash
cd packages/convex && vp run convex:env -- set WORKOS_API_KEY sk_your_key
cd packages/convex && vp run convex:env -- set WORKOS_CLIENT_ID client_your_id
cd packages/convex && vp run convex:env -- set OPENAI_API_KEY sk-your-key  # Optional, for semantic search
```
...
The sync API is defined in `packages/convex/src/http.ts`. These are the endpoints that plugins call:
...
<CardGroup cols={2}>
  <Card
    ...
    href="/hosting/env"
    description="Set up all required configuration"
  />

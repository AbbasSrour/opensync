...
<Steps>
  <Step title="Mark sessions as eval-ready">
    From the Sessions view, select sessions and click **Mark as Eval Ready**. This sets `evalReady:
    true` on the session, making it appear in the Evals tab.
  </Step>
  ...
  <Step title="Add expected output">
    For sessions you want to use as ground truth, write the expected output that the model should
    have produced.
  </Step>
  <Step title="Export">Select labeled sessions and export in your chosen format.</Step>
</Steps>
...
<Steps>
  ...
  <Step title="Select sessions">
    Use the checkboxes to select individual sessions, or **Select All** for the current filtered
    view.
  </Step>
...
<CardGroup cols={2}>
  ...
  <Card
    title="API Reference"
    href="/api/endpoints"
    description="Programmatic access to export and search"
  />
</CardGroup>

...
```bash
cd packages/api && vp run convex:env -- set WORKOS_API_KEY sk_your_api_key
cd packages/api && vp run convex:env -- set WORKOS_CLIENT_ID client_your_client_id
```
...
OpenSync's Convex backend uses the `@workos-inc/authkit-js` integration. The auth configuration is defined in `packages/api/convex/auth.config.ts`. No changes are needed for standard setups. Convex validates the WorkOS JWT on every function call automatically.
...
Check that the auto-provision logic in `packages/api/convex/auth.config.ts` is enabled. On first sign-in, the Convex auth middleware should create a user record automatically. If the `users` table is empty after a successful login, check the Convex function logs for errors.

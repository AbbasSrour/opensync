...
<Steps>
  ...
  <Step title="Save securely">Copy the key immediately. It won't be shown again.</Step>
</Steps>
...
<Info>We cannot recover your API key if you lose it. Generate a new one if needed.</Info>
...
<Warning title="Security recommendations">
  - Generate separate keys for development and production - Rotate keys periodically - Never commit
  keys to version control - Use environment variables to store keys - Revoke keys that may be
  compromised
</Warning>

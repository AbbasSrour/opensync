...
All backend logic lives in `packages/api/convex/` in this monorepo.
...
# Convex backend (set via cd packages/api && vp run convex:env -- set)
WORKOS_API_KEY=sk_...
...
Sync Plugin (npm package, runs locally)
    ...
    |
Convex HTTP Endpoints (packages/api/convex/http.ts)
    |
    v

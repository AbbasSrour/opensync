...
```bash
vp install -g opencode-sync-plugin
opencode-sync login
```
...
```bash
vp install -g cursor-sync-plugin
cursor-sync login
```
...
```bash
vp install -g pi-opensync-plugin
```
...
[![Deploy with Vercel](https://vercel.com/button)](https://vercel.com/new/clone?repository-url=https://github.com/waynesutton/opensync&env=VITE_CONVEX_URL,VITE_WORKOS_CLIENT_ID,VITE_REDIRECT_URI&envDescription=Required%20environment%20variables%20for%20OpenSync&envLink=https://github.com/waynesutton/opensync/blob/main/ONE-CLICK-DEPLOY.md)
